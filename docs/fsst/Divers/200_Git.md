...
### Aufgabenstellung
1. Erstelle ein **leeres** Repository auf GitHub (Name: `test-projekt` oder nach Wunsch).
2. Klone das Repository lokal.
3. Erstelle eine kleine Textdatei `README.md` mit einer kurzen Projektbeschreibung.
4. Führe nacheinander `git add`, `git commit` und `git push` aus, um diese Datei hochzuladen.
5. Schaue auf GitHub nach, ob die Datei `README.md` im Online-Repository sichtbar ist.
